...
const root=resolve('.artifacts/real-local-gpu');await mkdir(root,{recursive:true});
...
const report={capturedAt:new Date().toISOString(),evidence:'REAL_GGUF_WEBGPU_PRODUCTION_LIVE_ENGINE_SYNTHETIC_YOUTUBE_DOM',modelPath,source,checks:{},network:{syntheticFulfilled:[],blocked:[]},samples:[],errors:[],warmGenerations:[],limitations:['Synthetic native YouTube DOM, not actual YouTube or actual paid messages.','One cold and three later generations are not steady-state/P95 performance.','CPU times and process working sets cover this isolated Chromium instance; GPU execution is established by actual native worker hardware device and full native layer offload, not physical utilization counters.']};
...
async function addRows(count,prefix,kind='paid',text=source){
  ...
  },{count,prefix,kind,source:text});
}
async function translatedRows(prefix,count,timeout=20000,originalText=source){
  return until(async()=>{
    ...
    // This verifies protocol/state and changed output only, not language/translation quality.
    return rows.length===count&&rows.every(row=>row.state==='translated'&&row.text.trim()&&row.text!==originalText)?rows:null;
...
try{
  ...
  report.loaded=await state();assert.equal(report.loaded.gpu?.verified,true);assert.equal(report.loaded.gpu.offloadedLayers,report.loaded.gpu.totalLayers);assert.ok(report.loaded.gpu.offloadedLayers>0);report.checks.nativeHardwareGpuFullOffload=true;
  ...
  for(const [index,text] of ['応援しています。','今日も配信してくれてありがとう。','次の動画も楽しみにしています。'].entries()){
    const before=await state(),start=Date.now();await addRows(1,`warm${index}-`,'paid',text);
    const rows=await translatedRows(`warm${index}-`,1,120000,text);const after=await state();
    assert.equal(after.inferenceCalls,before.inferenceCalls+1);assert.equal(after.gpu?.verified,true);
    report.warmGenerations.push({source:text,text:rows[0].text,durationMs:Date.now()-start,inferenceCalls:after.inferenceCalls});
    console.log('WARM',JSON.stringify(report.warmGenerations.at(-1)));
  }
  report.checks.threeLaterDistinctRealGpuGenerations=true;
  const retainedJapanese=report.warmGenerations.filter(row=>/[\u3040-\u30ff]/u.test(row.text));
  report.quality={status:retainedJapanese.length?'PARTIAL':'NOT_INDEPENDENTLY_GRADED',note:'Transport/state checks are not language quality acceptance; retained Japanese is a bounded heuristic requiring review.',retainedJapanese};
  await sample('after-three-warm');report.final=await rpc({type:'overview'});assert.equal((await state()).inferenceCalls,4);
